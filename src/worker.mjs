...
async function handleRequest(request, env, ctx) {
  ...
  if (pathname === "/profiles/sync-from-google" && request.method === "POST") {
    ...
    for (const loc of locations) {
      ...
      const profile = {
        ...
        disabled:
          typeof existingProfile.disabled === "boolean"
        ...
        placeReviewUri:
          metadata.placeReviewUri || existingProfile.placeReviewUri || ""
      };
  ...
  if (pathname === "/post-now" && request.method === "POST") {
    ...
    if (!profileId) return jsonResponse({ error: "Missing profileId" }, 400);
    const { postToGmb } = await import("./gmb.mjs");
